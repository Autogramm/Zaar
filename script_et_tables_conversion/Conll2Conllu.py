...
# feats = dic[cle][1]
# gloss = dic[cle][2]
# "cle" étant un element de la colonne "Split GE" ou "Split RX"
# --------------------------------------------------------------------------
...
def fichiersAmettre():
    dossierConllu = input("Dossier conllu à traiter : ")
    ...
    return dossierConllu, fichiertableGE, fichiertableRX, fichiertablePonctuation
# --------------------------------------------------------------------------
# Fonction principale de la conversion
def conversion():
    # on crée nos dictionnaires
    if bonsFichiers():
        dossierConllu = "./SAY_BC_23mars"
        ...
        try:
            dossierConllu, table_GE, table_RX, table_PUNCT = fichiersAmettre()
            dict_GE, dict_RX, dict_PUNCT = makeOneDict(table_GE), makeOneDict(table_RX), makeOneDict(table_PUNCT)           
        except:
            print("fichiers non trouvés")
            
    files = os.listdir(dossierConllu)
    for fichierConllu in files:
        print(fichierConllu)
        with open("./"+dossierConllu+"/"+fichierConllu, "r", encoding="utf-8") as inputConllu, open("./resultat_conversion_conllu/"+"conversion"+fichierConllu, "w", encoding="utf-8") as outputConlluUD:
                    ...
                    if estPunct == False:
                        ...
                        for cleGE in dict_GE.keys():
                            for elem in trait:
                                ...
                                if cleGE == elemClean:
                                    # le pos de RX peut être écrasé par de GE s'il y en a un
